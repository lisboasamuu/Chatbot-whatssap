...
function Access({company,onSaved}:{company:PlatformCompanyDetail;onSaved:(c:PlatformCompanyDetail)=>void}) {
  const [email,setEmail]=useState(company.access.email??''),[password,setPassword]=useState(''),[msg,setMsg]=useState('');
  async function save(){try{onSaved(await api.saveCompanyAccess(company.id,{email,password}));setPassword('');setMsg(company.access.configured?'Acesso atualizado. Sessões anteriores foram revogadas.':'Acesso empresarial provisionado.');}catch(e){setMsg(e instanceof Error?e.message:'Erro ao salvar acesso.');}}
  return <Card><h3 className="font-semibold">Acesso da empresa</h3><p className="mt-1 text-sm text-zinc-500">Credenciais usadas no dashboard empresarial. A senha nunca é exibida depois de salva.</p><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="text-sm text-zinc-400">E-mail<input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="field"/></label><label className="text-sm text-zinc-400">{company.access.configured?'Nova senha (opcional)':'Senha inicial'}<input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="field" placeholder={company.access.configured?'Deixe vazio para manter a atual':'Mínimo de 12 caracteres'}/></label></div><button onClick={()=>void save()} className="mt-5 rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-semibold text-black">Salvar acesso</button><SaveFeedback message={msg}/></Card>;
}
